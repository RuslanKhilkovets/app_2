export {default as PrivacyPolicyScreen} from './PrivacyPolicyScreen';
export {default as EmailConfirmationScreen} from './EmailConfirmationScreen';
export {default as ResetPasswordScreen} from './ResetPasswordScreen';
export {default as SignFormsScreen} from './SignFormsScreen';
export {default as StartScreen} from './StartScreen';
export {default as StartSliderScreen} from './StartSliderScreen';
export {default as MainScreen} from './MainScreen';
export {default as About} from './About';
export {default as ItemScreen} from './ItemScreen';

// change data screens
export {default as ActivePublicationsScreen} from './changeDataScreens/ActivePublicationsScreen';
export {default as ChangeNameScreen} from './changeDataScreens/ChangeNameScreen';
export {default as ChangeLocationScreen} from './changeDataScreens/ChangeLocationScreen';
export {default as ChangePhoneScreen} from './changeDataScreens/ChangePhoneScreen';
export {default as ChangeEmailScreen} from './changeDataScreens/ChangeEmailScreen';
export {default as ChangePasswordScreen} from './changeDataScreens/ChangePasswordScreen';

// tabs
export {default as AddItemTab} from './tabs/AddItemTab';
export {default as ChatsTab} from './tabs/ChatsTab';
export {default as ProfileTab} from './tabs/ProfileTab';
export {default as FavoritesItemsTab} from './tabs/FavoritesItemsTab';
export {default as SearchTab} from './tabs/SearchTab';
